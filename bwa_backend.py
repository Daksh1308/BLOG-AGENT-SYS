...
# -----------------------------
# 4) Research (Tavily — with DuckDuckGo fallback)
# -----------------------------
            ...
            out.append(
                {
                    ...
                    "snippet": r.get("description") or r.get("body") or "",
                    "published_at": None,
                    "source": None,
                }
            )
...
def _generate_image_bytes(prompt: str) -> bytes:
    ...
    draw.rectangle([(0, 0), (width, 80)], fill=(30, 60, 114))
    ...
    draw.text((40, 22), "Blog Diagram", fill=(255, 255, 255), font=font_title)
    ...
    y = 120
    draw.text((40, y), "Prompt:", fill=(100, 100, 100), font=font_body)
    y += 30
    ...
    y = max(y + 30, 400)
    accent = (30, 60, 114)
    draw.rounded_rectangle([(40, y), (width - 40, y + 90)], radius=12, outline=accent, width=2)
    draw.rounded_rectangle([(60, y + 15), (width - 60, y + 75)], radius=8, fill=(230, 238, 250))
    draw.text((width // 2 - 60, y + 30), "generated diagram", fill=accent, font=font_body)
...
def generate_and_place_images(state: State) -> dict:
    ...
    for spec in image_specs:
        ...
        try:
            ...
            tag = f"![{spec['alt']}](data:image/png;base64,{__import__('base64').b64encode(img_bytes).decode()})\n*{spec['caption']}*"
            md = md.replace(placeholder, tag)
